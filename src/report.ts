...
export const SYM: Record<string, string> = {
  ...
  b: "b", h: "h", t_wall: "t_wall", pil_pos: "pilasterplass.",
...
export const INPUT_GROUPS: { title: string; items: InputMeta[] }[] = [
  ...
  { title: "Bolter", items: [
    ...
    M("anchor", "Endeforankring", "", "endeforankring: mutter eller ankerplate",
      "sel", ["mutter", "plate"]),
    M("a_anch", "Ende/plate a₁", "mm", "bredde endeforankring (mutter/skive- el. platebredde)"),
...
export function buildReport(g: Inputs, R: Results): DocGroup[] {
  ...
  D("a_spr,mut", `${g.a_anch}+2·${g.e_h}`, `${f0(R.a_spread_n)} mm`);
  D("a_spr,pl", `min(${g.b}; ${g.a1p}+2·${g.e_h})`, `${f0(R.a_spread_p)} mm`);
  D("T_mutter", `¼(1−${g.a_anch}/${f0(R.a_spread_n)})·${g.N_t}`, `${f1(R.T_nut)} kN`);
